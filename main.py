...
# --- SMART COOKIE GENERATOR (Auto-Fix Spaces to Tabs) ---
def setup_cookies():
    if not COOKIES_ENV or len(COOKIES_ENV) < 10:
        return

    try:
        logger.info("🔧 Fixing Cookies format...")
        cleaned_lines = []
        cleaned_lines.append("# Netscape HTTP Cookie File")
        cleaned_lines.append("# This file is generated by the bot. Do not edit.")
        ...
        for line in raw_lines:
            ...
            # Valid cookie line has at least 7 parts
            # domain, flag, path, secure, expiry, name, value
            if len(parts) >= 7:
                # Reconstruct using TABS (\t)
                domain = parts[0]
                flag = parts[1]
                path = parts[2]
                secure = parts[3]
                expiry = parts[4]
                name = parts[5]
                value = "".join(parts[6:]) # Join rest in case value broke
        ...
        logger.info(f"✅ Cookies successfully repaired and saved to {COOKIE_FILE}")
        
    except Exception as e:
        logger.error(f"❌ Cookie Setup Error: {e}")

# Run Cookie Setup Immediately
setup_cookies()
        ...
        if "terabox" in url or "terashare" in url or "1024tera" in url:
            ...
            if "teraboxurl" in url:
                 try:
                     ...
                     if "surl=" in resp.url:
                         match = re.search(r'surl=([a-zA-Z0-9_-]+)', resp.url)
                         if match: return f"https://www.terabox.com/s/1{match.group(1)}"
                 except: pass
...
def download_media(url, mode='best'):
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if not info:
                return None, None, None, None, None, "Extraction Failed (Login/Captcha)"
            ...
            if not os.path.exists(filename):
                 return None, None, None, None, None, "File Missing after download."
