...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_data["premium"] and user_data["count"] >= DAILY_LIMIT:
        await update.message.reply_text(f"🚫 **Daily Limit Reached!** ({DAILY_LIMIT}/day).")
        return
    ...
    if file_path and os.path.exists(file_path):
        try:
            ...
            if file_size > MAX_FILE_SIZE:
                await wait_msg.edit_text("❌ File too large (>50MB).")
                os.remove(file_path)
                return
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"📊 Users: {total_users} | Downloads Today: {total_downloads}")

async def add_premium(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID: return
    try:
        update_user_stat(context.args[0], "premium", True)
        await update.message.reply_text("✅ User set to Premium.")
    except:
        await update.message.reply_text("Usage: /addpremium <id>")

async def remove_premium(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID: return
    try:
        update_user_stat(context.args[0], "premium", False)
        await update.message.reply_text("User removed from Premium.")
    except:
        await update.message.reply_text("Usage: /removepremium <id>")
...
async def setup_bot():
    ...
    if webhook_info.url != expected_url:
        await ptb_application.bot.set_webhook(url=expected_url)
